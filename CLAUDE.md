...
- `Validators/` is a category of its own — split out from `Utilities/Helpers/` once more than one
  ...
  immediately unless `paymentMethod['saveFutureUsage'] === true`, and only then requires
  ...
  "the `aliasId` constructor argument." Matching tests for both validators in `tests/Validators/`.
...
- `src/Services/` (PRE-3576, extended PRE-3587) holds application services that use `Auth/`'s JWT
  ...
  pattern a third time. `UnifiedApiPaymentService` (`final class`, extends `AbstractUnifiedApiService`) exposes two public
  methods. `getPayment(string $paymentId): array{status: int, body: string}` — GETs `<baseUrl>/payments/<paymentId>` and returns the raw HTTP response. It does not
  ...
  `<baseUrl>/payments` via `sendPostJson($url, $dto->createPayloadBody())`, after dispatching to whichever validator
