 ...
 * environment — requires VPN access, so this suite never runs in CI (same as
 * UnifiedApiPaymentServiceTest, see that file). Run locally via `make test-integration` with the
 * env vars below set (see `.env.example`).
 *
 * UPC_IT_HF_TOKEN needs a *freshly minted* hosted-fields token before each run: unlike
 * UPC_IT_PAYMENT_ID (a static fixture), an hfToken is single-use and short-lived — it comes from
 * actually driving the hosted-fields JS SDK in a browser, which this suite cannot do itself.
 */
final class UnifiedApiHostedPaymentServiceTest extends TestCase
{
    public function testCreateHostedPaymentCreatesARealPaymentFromAFreshHfToken(): void
    {
        $env = $this->requireEnv([
